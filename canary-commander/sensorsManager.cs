...
/*
  ...
  public static class sensorsManager
  {
    ...
    public static void deviceArrival(YModule m)
    {
      try
      {
        ...
        for (var i = 0; i < count; i++)
        {
          ...
          else if (ftype == "SerialPort")
          {
            YSerialPort serialPort = YSerialPort.FindSerialPort(serial + "." + fid);
            serialPort.registerValueCallback(sensorsManager.serialCallback);
          }
        }
    ...
    static ulong lastSMSsent = 0;
    static YSerialPort activeSerialPort = null;

    public static void serialCallback(YSerialPort p, string newValue)
    {
      activeSerialPort = p;
    }

    public static void handleRS485Input(YSerialPort p)
    {
      ...
      for (int i = 0; i < messages.Count; i++)
      {
        ...
        // Filter out I/O records to keep only less frequent messages
        if (fields.Length >= 2 && fields[1] != "io")
        {
          LogManager.Log(messages[i]);
          if (fields[1] == "alarm")
          {
            ...
            // If a Yocto-Hub-GSM-4G is connected
            ...
            if (!(mbox is null))
            {
              ...
              {
                // FIXME: Put your mobile phone number below and uncomment the line
                // mbox.sendTextMessage("+xxxxxxxxxx", messages[i]);
                lastSMSsent = YAPI.GetTickCount();
              }
              // Generate an HTTP callback over the GSM link
              YNetwork net = YNetwork.FindNetwork(mbox.get_serialNumber() + ".network");
              if (net.isOnline() && net.get_readiness() == YNetwork.READINESS_WWW_OK)
              {
                // FIXME: Put your HTTP callback URL below
                string args = "time=" + YAPI.GetTickCount() + "&csv=" + messages[i];
    ...
    public static void run()
    {
      ...
      try
      {
        ...
        if(!(sensorsManager.activeSerialPort is null))
        {
          handleRS485Input(activeSerialPort);
